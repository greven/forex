defmodule Forex do
  ...
  > ### From the [European Central Bank](https://www.ecb.europa.eu/stats/policy_and_exchange_rates/euro_reference_exchange_rates/html/index.en.html) {: .info}
  >
  ...
  * `:feed_fn` - An `mfa` tuple that can be used to fetch the exchange rates from a custom feed.
  ...
  The `Forex` library also provides mix tasks to export the exchange rates
  to a JSON file. You can use the following tasks:
  * `mix forex.export.current` - Export the current rates to a JSON file.
  * `mix forex.export.historic` - Export the historic rates to a JSON file.
  * `mix forex.export.ninety` - Export the last ninety days rates to a JSON file.

  The tasks accept some options to customize the output:
  ...
  * `--help`      - Show this help message.
  ...
  For example the task `mix forex.export.current --base USD --symbols EUR,USD,GBP`
  will export the current exchange rates for EUR, USD and GBP to a `current_rates.json`
  JSON file in the `priv/data/forex` directory with the base currency set to USD.
  """
