defmodule Forex.Currency do
  ...
  @doc """
  Get all list of all currencies.
  """
  def all(keys \\ :atoms)
  ...
  def all(:strings) do
    ...
    |> Map.new(fn {code, currency} ->
      {Helper.stringify_code(code), currency}
    end)
  end
  ...
  @doc """
  Get all list of all the disabled currencies.
  """
  ...
  def get(iso_code) when is_atom(iso_code) do
    iso_code = Helper.atomize_code(iso_code)
  ...
  @doc """
  Get the currency information for the given ISO code.
  If the currency is not found, it will raise a `Forex.CurrencyError`.
  """
  def get!(iso_code) do
  ...
  def exchange(amount, from, to, opts \\ [])
    ...
    with from <- Helper.stringify_code(from),
         to <- Helper.stringify_code(to),
      ...
      result =
        result
        |> Helper.round_value(round)
        |> Helper.format_value(format)
